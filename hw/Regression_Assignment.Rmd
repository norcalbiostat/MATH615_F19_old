...
3. Log-Linear Regression: $log(Q) \sim X + Z$
    - The explanatory varible $X$ can be binary or quantitative. 
...
The p-value for the estimate of the regression coefficient for `wakeup` is significant (b1=-488, p= 0.001). There is reason to believe that the time you wakeup is associated with your income. 
...
> After adjusting for the potential confounding factor of **gender**, **wake up time** ($\beta_{1}$ = `-611, (95% CI -904, -318), p<.0001`) was **significantly** and **negatively** associated with **income**. Approximately `3.2%` of the variance of **income** can be accounted for by **wake up time** after controlling for **gender**. Based on these analyses, **gender** _is not_ a confounding factor because the association between **wake up time** and **income** _is still_ significant after accounting for **gender**.
