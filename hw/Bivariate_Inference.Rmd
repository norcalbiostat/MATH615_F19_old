...
In this assignment you will practice three different types of bivariate analysis: 

* Quantitative Outcome ~ Categorical Explanatory == ANOVA
* Categorical Outcome ~ Categorical Explanatory == $\chi^{2}$ test of association
* Quantitative Outcome ~ Quantitative Explanatory == Linear regression analysis
...
4. Perform an appropriate statistical analysis using the full five step method as outlined in class and described below. 
    - Define the parameters being tested. ($\rho$, $p_{1}$, $\mu_{1}$, etc)
...
4. **Perform an appropriate statistical analysis.**
    ...
    I. 
Let $\mu_{i}$ be the true mean petal length for iris species $i$. 
...
A chi square ($\chi^{2}$) test of independence compares frequencies of one categorical variable for different values of a second categorical variable. The null hypothesis is that the relative proportions (values) of one variable are “independent” of the second variable; in other words, the proportions of one variable are the same for different values of the second variable. The alternate hypothesis is that the relative proportions of one variable are associated with the second variable
...
* Gender = categorical explanatory variable (variable `female_c`)
...
Is there a relationship between gender and self-reported general health?

* Null Hypothesis: There is no relationship between gender and self-reported general health.  
* Alternate Hypothesis: There is a relationship between gender and self-reported general health. 
...
Linear regression models can be used to evaluate whether there is a linear relationship between two numerical variables. When only one explanatory variable x is considered, this is termed simple linear regression (SLR). The basic idea of SLR is to use data to fit a straight line that relates the response Y to the predictor X.  The mathmatical relationship between x and y is written as 
...
Does the length of the flower's sepal affect the length of the flower's petal? 
