...
read_excel("schedule_615.xlsx", sheet="weekly_schedule") %>% 
  ...
  select(Reading:`Collaborative Notes`) %>% 
  pivot_longer(everything(), names_to = 'Content Type') %>%
  mutate(Item = gsub("\r\n", "-", value)) %>% 
  select(-value) %>%
  na.omit() %>%
  separate_rows(Item, sep = "-") %>% 
  arrange(`Content Type`) %>% 
...
* To each other
* Code of conduct
* To your learning tools and platforms (navbar)

* [lec00: Intro to the class](lecture/lec00_intro_class.html)

## Data Architecture

* What is data, where can it be found, how do we use it? 
* [Data architecture](https://hackmd.io/@norcalbiostat/01_data_arch) notes on HackMD
