...
cdat   <- rawdat %>% mutate(Date = format(date, "%m/%d"), 
                            Topics = topic, Materials=materials, Evaluation = eval) %>% 
              select(Week, Date, Topics, Materials, Evaluation, topic, eval, materials)
...
out <- select(cdat, Week, Date, Topics, Materials, Evaluation)
